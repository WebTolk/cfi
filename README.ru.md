...
Если включён экспорт тегов, ID тегов записываются в столбец `articletags`. При импорте в этом же столбце можно указать ID тегов через запятую:
...
Если в окне экспорта включены раскрытые поля `images` и `attribs`, вложенные параметры материала записываются как обычные CSV-столбцы. Имя столбца сохраняет префикс `article`, например `show_title` превращается в `articleshow_title`, а `article_layout` — в `articlearticle_layout`:

| articleid | articletitle | articleshow_title | articleshow_tags | articlearticle_layout |
| --- | --- | --- | --- | --- |
| 5 | О компании | 1 | 1 | cassiopeia:blog |
| 8 | Контакты | 0 | 0 |  |
