...
(CfiUpload => {
    ...
    /**
     * Сбрасываем вид области загрузки к первоначальному
     */
    CfiUpload.resetArea = () => {
        if (CfiUpload.dragZone.classList.contains('shadow-danger')) {
            CfiUpload.dragZone.classList.remove('shadow-danger');
    ...
    CfiUpload.uploadError = (response) => {
        // Эта часть вызывается только в случае если отвалился сервер с 500-й
        console.log(response);
    }
...
//
//     let CfiUploadModal = document.getElementById('addImageToCanvasModal');
//     WTProductBuilderApp.addImageToCanvasModal = CfiUploadModal;
//     CfiUploadModal.addEventListener('shown.bs.modal', event => {
//         CfiUpload.init();
//     })
//     CfiUploadModal.addEventListener('hide.bs.modal', event => {
//         CfiUpload.resetArea();
//     })
//
// });